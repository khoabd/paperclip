...
- For post-deploy heartbeat checks, query `GET /api/health/smoke` with board or agent auth and require both `checks.auth.status=pass` and `checks.migrations.status=pass`.
