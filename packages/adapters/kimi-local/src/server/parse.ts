...
export function parseKimiJsonl(stdout: string) {
  ...
  for (const rawLine of stdout.split(/\r?\n/)) {
    ...
    if (role === "assistant") {
      ...
      const contentArr = Array.isArray(event.content) ? event.content : [];
      for (const partRaw of contentArr) {
        const part = parseObject(partRaw);
...
const KIMI_AUTH_REQUIRED_RE =
  /(?:not\s+authenticated|please\s+authenticate|api[_ ]?key\s+(?:required|missing|invalid)|authentication\s+required|unauthorized|invalid\s+credentials|not\s+logged\s+in|login\s+required|run\s+`?kimi\s+auth(?:\s+login)?`?\s+first)/i;
